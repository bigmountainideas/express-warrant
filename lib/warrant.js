...
Warrant.prototype.enforceUserSession = function(req, res, next){
  if( req.session && req.session.user){
    next();
  }else {
    var origin_url = url.format({
      protocolis: req.protocol,
      host: req.get('host'),
      pathname: req.path,
      query: req.query
    });
    res.redirect('https://access.dev.entertainwe.com/login?redirect_uri='+encodeURIComponent(origin_url))
  }
};
